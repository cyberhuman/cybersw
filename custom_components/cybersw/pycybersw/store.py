from .model import CyberswitchAdvertisementData, CyberswitchDeviceState
...
class CyberswitchStateStore:
    def __init__(self, adv_data: CyberswitchAdvertisementData) -> None:
        self.current = CyberswitchDeviceState()
        self._adv_data = adv_data

    def patch(self, state: CyberswitchDeviceState) -> bool:
        updated_props = [
            p
            for p in dir(state)
